...
// Reset
  ...
  static addToCart({
    ...
    const existingIndex = Cart.cart.findIndex(
      ...
      && JSON.stringify(item.removed_ingredients) === JSON.stringify(removed_ingredients)
    );
  ...
  static toggleCartBadge(){
    const cartCount = document.getElementById('cartCount')

    if(!cartCount) return
    
    if (Cart.cart.length == 0) cartCount.classList.remove('bg-success')
    else cartCount.classList.add('bg-success')
  }
